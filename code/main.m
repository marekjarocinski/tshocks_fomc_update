% This script updates the shocks from Jarocinski (2024) JME
...
ttab = readtimetable(pathin + "fomc_surprises_jk.csv");
...
Y = ttab{:,ynames}*100;
ymaturities = [1/12 2 10 NaN]; % for plotting later
[T, N] = size(Y);
nv = N; % 1 or N
...
save('maxl','maxl');
write_shocks_aggregated(maxl.U1s, ttab.start, pathout + "U1s");
write_shocks_aggregated(maxl.U1bp, ttab.start, pathout + "U1bp");
...
fh = plot_resp(diag(std(maxl.U))*Cm, diag(std(maxl.U))*Cl, diag(std(maxl.U))*Cu, ymaturities, variableNames=ynames, RowLabels="u"+(1:N), showTitles=false);
exportgraphics(fh, pathout + "U1s_impact.jpg")
fh = plot_resp(temp\Cm, temp\Cl, temp\Cu, ymaturities, variableNames=ynames);
